...
<div class="page-holder">
    ...
    <div class="container">
        ...
        <section class="py-5">
            <!-- BILLING ADDRESS-->
            <h2 class="h5 text-uppercase mb-4">Hóa đơn chi tiết</h2>

            <!-- Form để chọn ID hóa đơn -->
            <form action="/chi-tiet-hoa-don/${userId}" method="get">
...
<%--                    <th scope="col">ID HoaDon</th>--%>
                    ...
                    <th scope="col">Trạng thái trả hàng</th>
                    <th scope="col">Nút bấm</th>
                </tr>
                </thead>
                <tbody>
                <c:forEach items="${chiTietHoaDon}" var="hd">
                    <tr>
                        <td>${hd.id}</td>
<%--                        <td>${hd.idHoaDon.id}</td>--%>
                        <td>${hd.soLuong}</td>
                        <td>${hd.idChiTietSanPham.id}</td>
                        <td>${hd.idChiTietSanPham.ten}</td>
                        <td>${hd.idChiTietSanPham.idKichCo.ten}</td>
                        <td>${hd.donGia}</td>
<%--                        <td>${hd.donGia * hd.soLuong}</td>--%>
                        <td>${hd.idHoaDon.ngayKhachNhan}</td>
                        <td>${hd.donGia * hd.soLuong}</td>
                        <td>
                            <c:choose>
                        ...
                        </td><%--                        <td><a href="">Trả Hàng</a></td>--%>

<%--                        <c:if test="${hd.trangThai == 4}">--%>


<%--                        <td><a href="/tra-hang/${hd.id}">Yêu cầu trả hàng </a></td>--%>
<%--                        <td>--%>
<%--                            <c:if test="${hd.trangThai == 1}">--%>
<%--                                <a href="/tra-hang/${hd.id}">Trả hàng</a>--%>
<%--                            </c:if>--%>
<%--                        </td>--%>
...
<%--                        </c:if>--%>
                    ...
                    <c:choose>
                        <c:when test="${hd.trangThai == 4}">
                            <!-- Không hiển thị gì cả nếu trạng thái bằng 4 -->
                        </c:when>
                        <c:otherwise>
                                <!-- Các cột thông tin khác của hóa đơn -->

                                <td>
                                    <a href="/tra-hang/${hd.id}"><p style="font-size: small">Yêu cầu trả hàng</p></a>

                                    <c:if test="${hd.trangThai == 1}">
                                        <a href="/tra-hang-soluong/${hd.id}"><p style="font-size: small">Trả hàng</p></a>
                                    </c:if>
                                </td>
                        </c:otherwise>
                    </c:choose>
                ...
                <tr>
                    <td colspan="7"></td> <!-- Để cân bằng số cột với hàng trên -->
                    <td>
                        <c:set var="total" value="0" />
                        <c:forEach items="${chiTietHoaDon}" var="hd">
                            <c:set var="total" value="${total + (hd.donGia * hd.soLuong)}" />
                        </c:forEach>
                        ${total}
                    </td>
                </tr>
